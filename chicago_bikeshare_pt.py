...
while i <= 20:
    ...
    i += 1
"""
for line_index, line_enume in enumerate(data_list[:21]):
    print(f"Line {line_index}: {line_enume}")
...
for line_item, line_enume in enumerate(data_list[:20]):
    print(f"Line {line_item}: {line_enume[-2]}")
...
def count_items(column_list):
    ...
    count_items = [column_list.count(items) for items in item_types]
...
def count_type_user(data_list):
    """
    Função de contagem dos tipos de gênero presente nos dados - nulo, male e female.
    Argumentos:
        gender_nulo: gêneros não declarados, ou seja, lugares da lista que que apresentam espaço vazio.
        gender_m: variável que armazena a quantidade de gêneros masculinos.
        gender_f: variável que armazena a quantidade de gêneros feminino.
    Retorna:
        uma lista com a quantidade total de cada gênero.
    """
